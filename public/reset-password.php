<?php
// Reset Password Page for Customers
session_start();
...
$token = $_GET['token'] ?? '';
$userType = $_GET['type'] ?? 'customer'; // 'customer', 'staff', or 'supplier'
...
// Validate user type
if (!in_array($userType, ['customer', 'staff', 'supplier'])) {
    header('Location: login.php');
    exit;
}

// Set appropriate redirect URLs based on user type
$redirectUrl = 'login.php'; // default
$apiEndpoint = '../backend/api/shop/auth.php?action=reset_password'; // default for customers

switch ($userType) {
    case 'staff':
        $redirectUrl = 'simple-login.php';
        $apiEndpoint = '../backend/api/auth/reset-password.php';
        break;
    case 'supplier':
    ...
    default:
        ...
        $apiEndpoint = '../backend/api/shop/auth.php?action=reset_password';
        break;
}
?>
...
<head>
    ...
    <title>Reset Password - PC Parts Central</title>
...
<body>
    <div class="auth-container">
        ...
        <div class="security-note">
            ...
            <p>Your new password will be securely encrypted and stored. Make sure to choose a strong, unique password that you haven't used elsewhere.</p>
        </div>
    </div>
