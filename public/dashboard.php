...
<body>
    ...
    <!-- Inactivity Monitor -->
    <script src="assets/js/inactivity-monitor.js?v=2.0"></script>
    ...
    <script>
        ...
        const INACTIVITY_BLUR_TIMEOUT = 10000; // 10 seconds - show blur overlay
        const INACTIVITY_LOGOUT_TIMEOUT = 120000; // 2 minutes - auto logout (60,000 ms)
        ...
        // Automatic logout countdown and overlay management
        function initAutoLogout() {
            ...
            logoutTimer = setTimeout(() => {
                performAutoLogout();
            }, INACTIVITY_LOGOUT_TIMEOUT);

            // Start blur overlay timer (much shorter)
            blurTimer = setTimeout(() => {
                showAutoLogoutWarning();
            }, INACTIVITY_BLUR_TIMEOUT);
        }
        ...
        function showAutoLogoutWarning() {
            ...
            // Update overlay message for auto-logout warning
            const messageDiv = overlay.querySelector('.blur-message');
            if (messageDiv) {
                    ...
                    <div class="mb-3">
                        <div id="countdown-container" class="d-flex justify-content-center align-items-center">
                            <div class="progress me-3" style="width: 120px;">
                                <div id="countdown-progress" class="progress-bar progress-bar-animated progress-bar-striped bg-danger" role="progressbar" style="width: 100%; background: linear-gradient(90deg, #dc3545 0%, #ff6b6b 100%) !important;"></div>
                            </div>
                            <div class="countdown-display" style="font-size: 1.8rem; font-weight: bold; color: var(--text-primary); font-family: monospace;">
                                <span id="countdown">1:00</span>
                            </div>
                        </div>
                    </div>
                    <p>You will be automatically logged out due to inactivity</p>
                    <p><small class="text-muted">Move your mouse or press any key to extend your session</small></p>
                `;
            }
            ...
            // Start countdown timer (55 seconds remaining = when blur overlay appears after 5 seconds)
            startCountdown(55); // Time remaining until auto-logout when blur appears
        }

        function startCountdown(seconds) {
            const countdownElement = document.getElementById('countdown');
            const progressElement = document.getElementById('countdown-progress');
            const messageDiv = document.querySelector('#blur-overlay .blur-message');
            ...
            const countdownInterval = setInterval(() => {
                ...
                // Add warning features 10 seconds before logout
                if (seconds <= 10) {
                    // Critical warning phase
                    countdownElement.style.color = '#ff0000';
                    countdownElement.style.fontSize = '2.5rem';
                    countdownElement.style.fontWeight = 'bold';
                    countdownElement.style.animation = 'pulse 0.5s infinite alternate';

                    // Change progress bar to warning red
                    if (progressElement) {
                        progressElement.className = 'progress-bar progress-bar-animated progress-bar-striped bg-warning';
                        progressElement.style.background = 'linear-gradient(90deg, #ffc107 0%, #ff6b35 100%) !important';
                    }

                    // Update message to URGENT WARNING
                    if (messageDiv) {
                        messageDiv.style.border = '3px solid #ff0000';
                        messageDiv.style.boxShadow = '0 0 30px rgba(255, 0, 0, 0.5)';
                        messageDiv.innerHTML = `
                            <i class="fas fa-exclamation-triangle fa-4x mb-3" style="color: #ff0000; animation: blink 1s infinite;"></i>
                            <h4 style="color: #ff0000; font-weight: bold;">⚠️ CRITICAL: SESSION EXPIRING NOW! ⚠️</h4>
                            <div class="mb-3">
                                <div id="countdown-container" class="d-flex justify-content-center align-items-center">
                                    <div class="progress me-3" style="width: 120px;">
                                        <div id="countdown-progress" class="progress-bar progress-bar-animated progress-bar-striped bg-warning" role="progressbar" style="width: ${((seconds / initialSeconds) * 100)}%; background: linear-gradient(90deg, #ffc107 0%, #ff6b35 100%) !important;"></div>
                                    </div>
                                    <div class="countdown-display" style="font-size: 2.5rem; font-weight: bold; color: #ff0000; font-family: monospace; animation: pulse 0.5s infinite alternate;">
                                        <span id="countdown">${minutes}:${remainingSeconds.toString().padStart(2, '0')}</span>
                                    </div>
                                </div>
                            </div>
                            <p style="color: #ff0000; font-weight: bold;">You will be logged out in ${seconds} second${seconds === 1 ? '' : 's'}!</p>
                            <p><small style="color: #ff6666; font-weight: bold;">⚠️ CLICK ANYWHERE TO STAY LOGGED IN IMMEDIATELY! ⚠️</small></p>
                        `;
                    }
        ...
        function performAutoLogout() {
            ...
            // Perform logout automatically
            logout(true); // Silent logout without confirmation
        }

        // Modified logout function to handle silent logout
        async function logout(silent = false) {
            if (!silent && !(await showConfirm('Are you sure you want to logout?', {
                title: 'Confirm Logout',
                confirmText: 'Logout',
                type: 'warning',
                icon: 'fas fa-sign-out-alt'
        ...
        // Replace the old inactivity monitoring with auto-logout
        function resetInactivityTimer() {
            // Reset auto-logout timers
            initAutoLogout();

            // Hide blur overlay if shown
            if (isBlurred) {
                hideBlurOverlay();
            }
        }
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Initialize auto-logout monitoring after authentication check
            if (typeof checkAuth === 'function') {
                // Wait for checkAuth to complete, then initialize monitoring
        ...
        // Reset on window focus, pause on blur
